...
const DashboardControls: React.FC<DashboardControlsProps> = ({
  ...
  const { data: students, isFetching: fetchingStudents } = useQuery<Student[]>({
    ...
    queryFn: async () => {
      const res = await api.getStudents(globalState.courseID!, 1, 1000, globalState.ferpaPrivacy)
  ...
  const selectedAssignmentPretty = useMemo(() => {
    ...
    const found = assignments?.find((a) => a.id === globalState.assignmentId)?.name
    return found ? truncateString(found, 20) : 'Unknown'
  ...
  const AssignmentDropdown = () => (
    <CustomDropdown
      icon="file"
      label={globalState.assignmentId ? selectedAssignmentPretty : 'Select Assignment'}
  ...
  return (
    <Card
      ...
      ref={elementRef}
    >
      <p className="tw:font-semibold tw:text-lg tw:text-center tw:mb-1 tw:mt-0">Filters</p>
