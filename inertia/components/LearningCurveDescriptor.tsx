import { LearningCurveData } from '#types/index'
...
const LearningCurveDescriptor: React.FC<LearningCurveDescriptorProps> = ({ data }) => {
  ...
  function drawChart() {
    ...
    const domain = data.score_data.map((d) => format(d.submission_date, 'MM/dd/yyyy'))
    ...
    svg
      ...
      .call(
        d3
          .axisBottom(x)
          .tickValues(x.domain().filter((d, i) => i % Math.ceil(domain.length / 8) === 0))
      )
          ...
          <div className="tw:flex tw:flex-row tw:justify-between">
            <div className="tw:flex tw:flex-col">
              <div className="tw:flex tw:flex-row tw:mb-0 tw:items-center">
                <h3 className="tw:text-2xl tw:font-semibold">{data.descriptor.text}</h3>
              </div>

              <p className="tw:text-xs tw:text-gray-500 tw:mt-0">
                Average performance across associated questions (
                <LearningObjectiveQuestionsAligned questions={data.descriptor.questions} />)
              </p>
            </div>
          </div>
