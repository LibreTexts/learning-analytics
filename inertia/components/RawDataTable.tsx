...
import { AnalyticsRawData } from '#types/index'
...
const RawDataTable: React.FC<RawDataTableProps> = ({}) => {
  ...
  const { data, isFetching } = useQuery<AnalyticsRawData[]>({
    ...
    queryFn: async () => {
      const res = await api.getCourseRawData(
        globalState.courseID!.toString(),
  ...
  const defaultColumns = [
    columnHelper.accessor('name', {
      cell: (info) => (
        <div>
          <span className="tw:font-semibold">{truncateString(info.getValue(), 30)}</span>
        </div>
      ),
      header: 'Student ID',
      // @ts-ignore
    ...
    columnHelper.display({
      ...
      header: '% Submitted',
      cell: (info) => {
        const total = info.row.original.not_submitted + info.row.original.submitted
        ...
        <>
          {!globalState.courseLetterGradesReleased && (
            <Alert variant="warning">
              Final grades have not been released for this course. The data displayed here may not
              be accurate.
            </Alert>
          )}
