...
import { GradeDistribution as GradeDistributionType, VisualizationBaseProps } from '#types/index'
import {
  createColumnHelper,
  flexRender,
  getCoreRowModel,
...
const GradeDistribution: React.FC<VisualizationBaseProps<GradeDistributionType | undefined>> = ({
  ...
  const [parsed, setParsed] = useState<GradeBucket[]>([])
  ...
  const getMaxCount = useMemo(() => {
    const vals = parsed.map((d) => d.count)
  ...
  return (
    <div ref={containerRef}>
      ...
      {!loading && (!parsed || parsed.length === 0) && (
        <NoData
          width={width}
          height={height}
          msg={'Letter grades have not been released for this course yet.'}
        />
      )}
    </div>
  )
