...
import { SubmissionTimeline as SubmissionTimelineType, VisualizationBaseProps } from '#types/index'
...
const SubmissionTimeline: React.FC<SubmissionTimelineProps> = ({
  ...
  const [flattened, setFlattened] = useState<SubmissionTimelineTypeFlat[]>([])
      ...
      columnHelper.group({
        ...
        columns: [
          ...
          columnHelper.accessor('date', {
            cell: (info) => <div>{formatDate(info.getValue().toString(), DATE_ONLY_FORMAT)}</div>,
  ...
  useEffect(() => {
    ...
    const flattened = data.questions.reduce((acc, curr) => {
      curr.data.forEach((d) => {
    ...
    const sorted = flattened.sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime())

    setFlattened(sorted)
    setDueDate(data.due_date)
    setFinalSubDate(data.final_submission_deadline)
  }, [data])
  ...
  useEffect(() => {
    ...
    if (tableView) return
    drawChart()
  }, [width, height, flattened, tableView])

  const getMaxCount = useMemo(() => {
    const vals = flattened.map((d) => d.count)
  ...
  function drawChart() {
    ...
    const domain = flattened.map((d) => format(d.date, 'MM/dd/yyyy'))
    const dueDateFormatted = dueDate ? format(dueDate, 'MM/dd/yyyy') : ''
    const finalSubDateFormatted = finalSubDate ? format(finalSubDate, 'MM/dd/yyyy') : ''
    ...
    if (dueDateFormatted) {
      ...
      svg
        .append('line')
        .attr('x1', x(dueDateFormatted) ? (x(dueDateFormatted) as number) + x.bandwidth() : 0)
        .attr('x2', x(dueDateFormatted) ? (x(dueDateFormatted) as number) + x.bandwidth() : 0)
        .attr('y1', MARGIN.top)
  ...
  return (
    <div ref={containerRef}>
      ...
      {!loading && selectedAssignmentId && flattened?.length > 0 && (
      ...
      {!loading && selectedAssignmentId && (!data || flattened.length === 0) && (
