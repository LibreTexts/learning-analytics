...
import { TextbookInteractionsCount, VisualizationBaseProps } from '#types/index'
...
const TextbookEngagement: React.FC<TextbookEngagementProps> = ({
  ...
  return (
    <>
      ...
      {!loading && data?.length > 0 && <svg ref={svgRef} width={width} height={height}></svg>}
      {!loading && (!data || data.length === 0) && <NoData width={width} height={height} />}
    </>
  )
}
