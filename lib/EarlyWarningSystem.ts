import { debugEWS } from "@/utils/misc";
...
class EarlyWarningSystem {
  ...
  private async writeEWSCourseSummary(): Promise<boolean> {
    try {
      ...
      debugEWS("[writeEWSCourseSummary]: Starting aggregation...");
      ...
      debugEWS(`[writeEWSCourseSummary]: Finished writing course summaries.`);
      return true;
    } catch (err: any) {
      debugEWS(
  ...
  private async writeEWSActorSummary(): Promise<boolean> {
    try {
      ...
      debugEWS("[writeEWSActorSummary]: Starting aggregation...");
      ...
      debugEWS(`[writeEWSActorSummary]: Finished writing actor summaries.`);
      return true;
    } catch (err: any) {
      debugEWS(
  ...
  private _getActorStatusFromPrediction(
    ...
    if (prediction <= EWS_THRESHOLDS.DANGER) {
      return "danger";
    } else if (prediction <= EWS_THRESHOLDS.WARNING) {
      return "warning";
    } else {
      return "success";
    }
